...
const Navbar = ({tokenConfirm, logOut}) => {
    ...
    return (
        ...
        <div className='container'>
          ...
          </div></>) : (<><Link to="/" className='navbar-brand'><img src={logo} alt="Cabride app logo" width="150px" height="60px"/></Link>
          <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"><span className="navbar-toggler-icon"></span></button>
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
              <li className="nav-item"><Link className="nav-link active" aria-current="page" to="/">Home</Link></li>
              <li className="nav-item"><Link className="nav-link" to="/dashboard">Dashboard</Link></li>
              <li className='nav-item'><button className='nav-link btn btn-danger' onClick={logOut}>Logout</button></li>
            </ul>
          </div></>)}
        </div>
      </nav>
